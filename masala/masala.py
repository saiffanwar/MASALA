...
class MASALA:
    ...
    def __init__(self, model, model_type, x_test, y_test, y_pred, dataset, features, discrete_features, sparsity_threshold, coverage_threshold, starting_k, neighbourhood_threshold):
        self.model = model
        ...
        self.run_clustering(preload_clustering=False)
    ...
    def explain_instance(self, instance, plotting=True):
        if self.explanation_generator is None:
            self.explanation_generator = LLCExplanation(model=self.model, x_test=self.x_test, y_pred=self.y_pred, dataset=self.dataset, features=self.features, discrete_features=self.discrete_features, sparsity_threshold=self.sparsity_threshold, coverage_threshold=self.coverage_threshold, starting_k=self.starting_k, neighbourhood_threshold=self.neighbourhood_threshold)
        explanation = self.explanation_generator.generate_explanation(self.x_test[instance], instance)
        if plotting:
